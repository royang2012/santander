"""
deleted a lot unimportant features.
"""
import csv
import datetime
...
# indrel_1mes_dict = {'1.0': 'I', '3.0': 'I', '2.0': '2.0', '4.0': '4.0', 'P': 'P', 'R':'R', 'A':'A'}

# indrel_reduce_map = lambda x: indrel_1mes_dict[x]
...
if __name__ == "__main__":
    ...
    # train_fea_df['indrel_re'] = train_fea_df['indrel_1mes'].map(indrel_reduce_map)
    # vali_fea_df['indrel_re'] = vali_fea_df['indrel_1mes'].map(indrel_reduce_map)
    # pred_fea_df['indrel_re'] = pred_fea_df['indrel_1mes'].map(indrel_reduce_map)
    ...
    # pred_weight_i = 1.0/np.sum(vali_out_df, axis=1)
    train_fea_list = []
    train_out_list = []
    train_weight_list = []
    ...
    for i in range(0, 1200, 1200):
        model = runXGB(train_X, train_y, train_weight, 220, seed_val=i)
