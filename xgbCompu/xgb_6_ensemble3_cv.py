...
for depth in range(14, 16):
    p_pred = np.zeros((pred_fea_df.shape[0]/2,product_num))
    for num_round in range(100, 200, 50):
        for seed_num in range(0, 800, 800):
            # # # ********************************************* # # #
            ...
            # num_round = 200
            train_prod_ary = np.zeros([train_fea_df.shape[0]/2, 4])
            pred_prod_ary = np.zeros([pred_fea_df.shape[0]/2, 4])
            for iter_products in tqdm(range(0, product_num)):
                ...
                for i in range(0, train_fea_df.shape[0]/2):
                    train_prod_ary[i] = train_prod_info[2*i: 2*i+2, :].reshape(4)
                for i in range(0, pred_fea_df.shape[0]/2):
                    pred_prod_ary[i] = pred_prod_info[2*i: 2*i+2, :].reshape(4)
        ...
        save_name = '../input/pred_1218_'+ str(depth)+ str(num_round) + '.csv'
        np.savetxt(save_name, p_pred, delimiter=",")
        ...
        file_name = '../output/sub_161218_' + str(depth)+ str(num_round) + '.csv'
